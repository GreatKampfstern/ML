...
Für das Festlegen der Batch-Größe, sowie der Epochenanzahl gab es ein paar Faktoren zu beachten: Eine kleinere Batch-Größe von 16 ermöglichte eine häufigere Aktualisierung der Gewichte, was zu einer schnelleren Anpassung des Modells führte. Jedoch kann eine zu kleine Batch-Größe zu instabilen Gradienten führen. Eine größere Batch-Größe von 64 hingegen stabilisierte die Gradienten, verlangsamte jedoch die Lernrate des Modells, ist aber bei kleinen Datensets (wie das hier) nicht sehr sinnvoll. Mehr Epochen erlauben dem Modell, die Daten gründlicher zu lernen, erhöhen jedoch auch die Gefahr des Overfittings, wenn das Modell zu stark an die Trainingsdaten angepasst wird.

Um dies zu vermeiden, wurde zum einen Dropout bei der Modellarchitektur verwendet(s. oben) und zum anderen die Anzahl der Epochen auf 50 festgelegt und ein Early Stopping implementiert, welches nach fünf Epochen ohne Verbesserung des Loss der Validierungsdaten das Training beendet. 
Für die Batchgröße haben wir uns für 32 entschieden, da diese einen guten Kompromiss zwischen Stabilität und Geschwindigkeit bietet. Eine Batch-Größe von 32 ist groß genug, um stabile Gradienten sicherzustellen, während sie gleichzeitig eine effiziente und regelmäßige Aktualisierung der Modellgewichte ermöglicht.
...
Die Analyse zeigt, dass eine Kombination von `max_depth=10` und `n_estimators=250` zu den besten Ergebnissen führt, da sie eine ausgewogene Balance zwischen Genauigkeit und Stabilität des Modells bietet.
